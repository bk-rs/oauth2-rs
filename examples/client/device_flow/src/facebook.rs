/*
RUST_BACKTRACE=1 RUST_LOG=debug cargo run -p oauth2_client_device_flow_example --bin device_flow_facebook -- 'YOUR_CLIENT_ID' 'YOUR_CLIENT_SECRET'
*/
...
use oauth2_facebook::{FacebookProviderForDevices, FacebookScope};
...
async fn run(client_id: String, client_secret: String) -> Result<(), Box<dyn std::error::Error>> {
    let scopes = vec![FacebookScope::Email, FacebookScope::PublicProfile];

    let flow = Flow::new(IsahcClient::new()?, IsahcClient::new()?);
    let provider = FacebookProviderForDevices::new(client_id, client_secret)?;
