...
use crate::{BitbucketScope, AUTHORIZATION_URL, TOKEN_URL};

#[derive(Debug, Clone)]
pub struct BitbucketProviderWithWebApplication {
    client_id: ClientId,
...
impl BitbucketProviderWithWebApplication {
    pub fn new(
        client_id: ClientId,
        client_secret: ClientSecret,
        redirect_uri: RedirectUri,
...
impl Provider for BitbucketProviderWithWebApplication {
    type Scope = BitbucketScope;
...
impl ProviderExtAuthorizationCodeGrant for BitbucketProviderWithWebApplication {
    fn redirect_uri(&self) -> Option<&RedirectUri> {
        Some(&self.redirect_uri)
    }

    fn scopes_default(&self) -> Option<Vec<<Self as Provider>::Scope>> {
        Some(vec![BitbucketScope::Email, BitbucketScope::Account])
    }
...
mod tests {
    ...
    fn access_token_response() -> Result<(), Box<dyn error::Error>> {
        let provider = BitbucketProviderWithWebApplication::new(
            "CLIENT_ID".to_owned(),
